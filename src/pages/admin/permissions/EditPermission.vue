    ...
    setup() {
        ...
        onMounted(async () => {
            ...
            await permissionStore.getById(permissionId.value)
                    .then((permission: Permission) => Object.assign(form, permission))
        ...
        const updatePermission = () => {
            reset()
            loading.value = true
            permissionStore.update({id: permissionId.value, ...form})
                .then(() => router.push({ name: 'permissions.index' }))
                .catch((error: any) => {
        ...
        return {
            form, loading, errors, updatePermission
        }
    },
}
</script>
...
<template>
    ...
    <h2 class="text-4xl font-extrabold mt-4">Editar Permissão</h2>
    ...
    <form action="#" method="post" @submit.prevent="updatePermission">
        <div class="mt-4">
        ...
        <div class="mt-4">
            ...
            <input type="text" name="description" placeholder="Description" v-model="form.description"
                class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6 p-2"
                :class="{ 'has-error': errors.validations.hasOwnProperty('description') }">
        </div>
